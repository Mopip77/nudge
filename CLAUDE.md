...
`ui/AlbumBackdrop.kt`，自下而上：

1. **铺底层**：封面 Crop 铺满全屏 + `blur(48dp)` + 放大 1.18 倍
2. **清晰层**：原比例的方形封面，四向羽化
3. **压暗层**：全屏均匀压暗 + 顶栏处额外一道渐变

「中间清晰、上下模糊」**没法靠单个 `blur` 修饰符做到**——`BlurNode` 恒
`clip=true` 且裁到自己那层的排版矩形，一张图不可能只糊一部分。所以是
两张图叠加，交界处靠羽化而不是裁切。

铺底层要放大：模糊会让图像边缘向内收（采样超出边界的部分没有内容），
不放大四条边会透出一圈发虚的暗边。

#### 尺寸修饰符必须排在 `align` / `offset` **之前**

排在后面时，这两个修饰符已经把约束改成了 wrap-content，`size()` 拿到的
是被夹过的约束。**实测封面只铺到七成宽，两侧露出硬边**，正是设计上
最不能出现的那种「这里是一张正方形图片，外面是另一层背景」。

这个缺陷有个迷惑人的特征：**上下羽化照常生效**（那是绘制阶段的事，
不受约束影响），于是很容易误判成「只有横向羽化没写对」，在羽化参数上
白调半天。

肉眼分辨「边缘是硬的还是羽化的」不可靠，逐像素量横向亮度梯度即可：

```python
grads = sorted(((abs(vals[x+1]-vals[x]), x) for x in range(w-1)), reverse=True)[:5]
```

缺陷版峰值 20（x=155 与 x=925 两处对称出现，间距正好是七成屏宽），
修好后降到 8 且位置分散。

#### 清晰层四向都要羽化，不只上下

左右边缘正好落在屏幕两侧，乍看不需要羽化。但清晰层与铺底层在边缘处
**内容并不连续**（一个是原图，一个是放大 1.18 倍的裁切），不羽化会在
屏幕两侧显出一条竖直接缝。左右的跨度取上下的一半：两侧只要把接缝抹掉，
糊太宽会把封面主体吃掉一截。

### 模式切换只 crossfade，不动 blur 半径

歌词开关时，清晰层 alpha 淡出、压暗层加浓，两者一起过渡（300ms）。
**不对 `blur` 半径做动画**：半径每变一次都要重新生成模糊，全屏尺寸下
每帧重算会和歌词的逐行动画、手势识别抢同一帧的预算。

观感上两者几乎没有差别——两层的模糊度本来就差很多，crossfade 过去
就是「越来越糊」。`AlbumArt` 那个暂停模糊确实在动半径，但那是 52dp
的小图，全屏是另一回事。
...
背景要铺满整个窗口（含顶栏背后，否则顶上会留一条突兀的背景色），
但清晰封面按整屏居中的话会被顶栏挤得偏下，进度条与封面之间空出一大片。
所以往下偏移半个顶栏高度。

顶栏高度由 `onSizeChanged` 实测，**还要加上 `safeDrawing` 在顶部让出的
那段**（刘海／挖孔）：顶栏的实测高度不含它，而背景层是从窗口物理顶边
起算的，不补这一段封面会整体偏上。
...
- 封面模式下清晰封面**铺满整屏宽度且两侧无硬边**。肉眼分辨羽化与硬边
  很吃力，逐像素量横向亮度梯度：正常时峰值 <10 且位置分散，
  缺陷版会在对称的两点出现 20 左右的突跳（见「尺寸修饰符必须排在
  align/offset 之前」）。

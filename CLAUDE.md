...
判定精度**随系统版本而变**：One UI 5.1 上 `clear` 不删 lock 条目
（`dumpsys` 里 `id=16` 仍在），于是恒判为「设过」；而 One UI 8 / Android 16
上 `clear` 之后 `id` 真的变 **-1**、`mCropHint` 清零，能真正区分两档。
两边都不影响正确性——误差只偏向保守那一侧（最坏是多要一张恢复图）。

`RestorePlanTest` 用穷举拦着「继承态被改成写图」这个方向。

#### `clear()` 救不回原壁纸，所以「请用户选一张恢复图」这步去不掉

曾寄望于「`clear` 走的不是那个 signature 权限的 API，语义是重置到默认，
三星可能自己把原壁纸绑回来」——**实测否掉了**（S24 Ultra / Android 16）：

| 阶段 | `id` | `mCropHint` | 锁屏画面 |
|---|---|---|---|
| 基线（用户自设的静态图） | 134 | `(0,30-1080,2370)` | 原图 |
| 写入封面后 | 135 | `(0,0-1080,2340)` | 封面 |
| **`clear(FLAG_LOCK)` 后** | **-1** | `(0,0-0,0)` | **纯黑** |

原图没有回来，也没有回落成桌面壁纸——`clear` 就是字面意义的「清空」。
加上 Android 13+ 读不到原图（没法自动备份），两头都堵死了，
所以「设过独立锁屏壁纸」的用户**必须**自己指定一张恢复图。
**别再试图省掉这一步。**

注意区分 `notifyLockWallpaperChanged` 的 `type`：写入是 `type = 0`，
`clear` 是 `type = 7`，logcat 里可据此确认到底执行了哪个。
...
- 锁屏壁纸的五项。开关与耗时都能从 logcat 读出来，不必靠肉眼：
  ...
  1. **关闭功能后壁纸回到原样**（最重要的一条）。分别在
     「继承态」与「设过独立锁屏壁纸」两种情形下各测一次——前者要断言
     锁屏**仍跟随桌面壁纸**（改一次桌面壁纸看锁屏跟不跟），
     而不是被固定成一张图。

     不必只靠截图比对，`mCropHint` 是个客观指纹——恢复后它应当与基线
     **逐字节相同**（实测基线与恢复后都是 `(0,30-1080,2370)`，
     而被封面覆盖期间是 `(0,0-1080,2340)`）：

     ```bash
     adb shell dumpsys wallpaper | ag -u -A4 'mLastLockWallpaper state'
     ```

     两个会浪费时间的坑：

     - **广播会被 One UI 的 MARs 拦掉**（logcat 里 `skipped in
       RestrictedPackage`），而 `am broadcast` 仍返回 `result=0`，
       看起来像「发了没反应」。每次发广播**前**都要把应用切到前台
       （`am start -n com.nudge.app/.MainActivity`），
       退到后台就会被重新限制。

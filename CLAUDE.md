...
刚度与阻尼沿屏幕**自上而下单调递增**：顶部最软最弹，底部最硬最稳。

早先用 `abs(index - currentIndex)` 取无向距离，梯度以当前行为中心向两侧
对称扩散。那是错的：列表整体只往上走一个方向，物理上被拖拽的只能是
**后面的**行，最上面那行反倒该最先到位。对称梯度让已经快滑出视野的行
和刚进场的行拿到同样的刚度，表现为「最上面那条歌词往上弹也有阻尼」。

基准取**目标位置**（动画结束后落在第几行）而非实时位置：后者会让刚度
随自身动画状态变化，构成非线性反馈，容易自激振荡。目标位置是静态量，
每次换行只重算一次。

`LyricsAnimSpecTest` 断言的是**方向性**（单调不减、上下不对称），
不是具体数值——数值随观感调，方向性是这版的前提，写反了在代码里看不出来。

#### 当前行锚定在第 3 行，不是恒定居中

`anchorRow = 2`。居中不符合阅读的实际重心：注意力在「下一句是什么」，
正下方的预读区比正上方已唱过的行重要得多。上方留两行做上下文，再少
就会出现「刚唱完的那句立刻滑出视野」，换行时缺少来处。
...
淡入淡出要**短于**位移落定的时间：等长时换行途中新的当前行会
「边移动边对焦」，先建立焦点再收尾位移才对。位移整体调慢后这个值
也跟着抬了一档，但比例关系（约位移的 2/3）保持不变。
...
参数**只存内存，退出即丢**，也不进 `NudgeConfig`。歌词动画的好坏没有
「因人而异」的成分，做成用户可持久化的配置只会让线上出现一堆没人能复现的
观感问题。调好之后点「打印当前参数」，把可直接粘贴的构造调用抄回
`LyricsAnimSpec.DEFAULT`。不用剪贴板是因为真机调参时手边未必有键盘，
且剪贴板在分屏／后台限制下时灵时不灵。
...
107 个单元测试，主体在 `GestureRecognizer`——正例（五种手势 × 三档灵敏度）、边界（阈值临界）、负例（滑动、指数不符、超时）。**动手势逻辑必须补相应测试**，尤其是防误触的负例。

预设部分由 `ProfileCodecTest` 覆盖 round-trip 与宽容解码，`ProfileSlotTest` 覆盖槽位号解析。

歌词动画由 `LyricsAnimSpecTest` 覆盖，断言的是梯度的**方向性**而非具体数值
（见「梯度基准是屏幕位置」一节）。渲染本身没有自动化测试，靠实验室肉眼看。

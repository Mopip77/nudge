...
#### 默认取 **4:5 竖**，不是方形也不是更竖的比例

竖图明显更满：方图清晰区只占屏高 45%，4:5 占 56%。

不取 2:3 / 9:16 有两条实测理由：

1. **会掉分辨率。** 它们的长边先顶到原图上限——1400 的原图上，
   2:3 只能拿到 933×1400、9:16 拿到 788×1400，**铺不满 1080px 宽**，
   反而要上采样。4:5 拿的是 1080×1350，正好铺满。
2. **裁过头会切掉封面的字。** 居中裁切按比例吃掉上下，2:3 要裁原图的
   33%，而专辑的标题字通常就在上下三分之一处。

实测 2:3 在人像封面上确实更好看，但在「滚石30周年」这种文字型封面上
会把美术字顶进顶栏。4:5 是「明显比方图高」与「几乎不伤构图」的平衡点。

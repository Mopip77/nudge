...
enum class OriginalWallpaperKind {
    ...
    /**
     * 锁屏壁纸由一个 **live wallpaper 组件**渲染。**一律拒绝开启。**
     *
     * ## 「用户明明选的是一张照片」——这两件事不矛盾
     *
     * 真机上（S24 Ultra / One UI 8.5）锁屏壁纸是
     ...
     * ## 为什么必须拒绝
     *
     * 恢复这类壁纸要重新绑定那个 **service**，需要
     * `SET_WALLPAPER_COMPONENT`（signature 级），第三方拿不到；
     * 用户指定的「恢复图」也救不了——写回去的是一张平面图，
     * 特效没了。这一档**没有任何恢复手段**。
     *
     * 开发中真的这么覆盖过一次用户的锁屏壁纸。好消息是源图还在他相册里，
     * 可以自己重选；但「要用户自己去补救」本身就是不该发生的事。
     */
    LIVE_WALLPAPER,
}
